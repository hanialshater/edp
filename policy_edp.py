"""
EDP policy: PWL shape functions (Layer 1) -> 7 problem fingerprint
            -> module GAM (Layer 2) -> greedy submodular composition.

The policy is parameterized by:
  - PROBLEM_SHAPES: signal -> (breakpoints, values, weight)
  - MODULES: widget -> dict(addr, base, on_rem, on_cov, slot_decay, type)
...
from sim import N_SLOTS, WIDGETS
...
def _set_dot(d: dict, path: str, value: float):
    """Set a dotted path inside a module config dict. Creates intermediate dicts."""
    parts = path.split('.')
    cur = d
    for p in parts[:-1]:
...
def apply_edits(modules: dict, edits: list) -> dict:
    """
    edits: list of (widget, path, from_value_or_None, to_value, reason)
    Returns a new modules dict; original is not mutated.
    """
...
def load_edits_json(path: str) -> tuple[list, str]:
    with open(path) as f:
        data = json.load(f)
...
if __name__ == '__main__':
    from sim import make_session_stream
    pol = EDPPolicy()
    stream = make_session_stream(5, seed=42)
    for persona, feat in stream:
        page = pol.select_page(feat)
        print(f'{persona:22s} -> {page}')
