# Persona generation task — persona: `tabbed_comparison_shopper`
...
Has 4-7 tabs open across three different retailers. Knows what they want stylistically and is doing a final pricing pass. Low size anxiety (knows their measurements). Wishlist already exists from prior visits. Will compulsively check the price on each tab and look at the value-breakdown / shipping cost section. Often comes back the next day.

mixture_weight (share of stream): 0.11

## Required output

Write a JSON file at `/home/user/edp/data/persona_drafts/tabbed_comparison_shopper.json` with this EXACT schema:

```json
{
  "name": "tabbed_comparison_shopper",
