# Persona generation task — persona: `returner_from_recent_order`
...
Order from last week is being returned right now and they're browsing for a replacement. They know the brand, know their size (from the failure), and are slightly cautious. Their previous item didn't work, so they're more open to alternatives and to 'low return rate' recommendations. Mid revisit, high return_view.

mixture_weight (share of stream): 0.05

## Required output

Write a JSON file at `/home/user/edp/data/persona_drafts/returner_from_recent_order.json` with this EXACT schema:

```json
{
  "name": "returner_from_recent_order",
