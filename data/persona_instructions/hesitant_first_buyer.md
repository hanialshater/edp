# Persona generation task — persona: `hesitant_first_buyer`
...
First-time visitor on a friend's recommendation. Late twenties, has never bought from this brand before. Looking at a mid-price item but anxious about whether it'll fit, whether they can return it easily, and whether the brand is legit. Spends a lot of time reading the size chart, opens the returns policy in a new tab, hovers over price multiple times. Has not added to cart yet. Mobile device, evening.

mixture_weight (share of stream): 0.13

## Required output

Write a JSON file at `/home/user/edp/data/persona_drafts/hesitant_first_buyer.json` with this EXACT schema:

```json
{
  "name": "hesitant_first_buyer",
