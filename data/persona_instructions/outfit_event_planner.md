# Persona generation task — persona: `outfit_event_planner`
...
Has a specific event in mind — wedding, vacation, work conference. Looking at an item that needs to complete an outfit. High style- stretch (open to discovery), interested in 'wear it with' content and lookbooks. Less price-sensitive than average. Will linger on outfit-completion or occasion-lookbook content.

mixture_weight (share of stream): 0.09

## Required output

Write a JSON file at `/home/user/edp/data/persona_drafts/outfit_event_planner.json` with this EXACT schema:

```json
{
  "name": "outfit_event_planner",
