# Persona generation task — persona: `mobile_evening_browser`
...
On phone, on the couch, after dinner. Mobile-only signals high. Low engagement depth (doesn't zoom much, doesn't read long copy). Casually browsing, maybe will save to wishlist, low intent to buy today. Moderate style-stretch, low size anxiety, low compare activity.

mixture_weight (share of stream): 0.08

## Required output

Write a JSON file at `/home/user/edp/data/persona_drafts/mobile_evening_browser.json` with this EXACT schema:

```json
{
  "name": "mobile_evening_browser",
