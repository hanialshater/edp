# Persona generation task — persona: `bargain_hunter_returning`
...
Loyal returning customer who only ever buys on sale. Knows the brand well. Checks the price history widget if available, has saved items to a wishlist months ago, watches for price drops. Low size anxiety, low return anxiety. Will tab-switch but to check coupon sites, not other retailers.

mixture_weight (share of stream): 0.1

## Required output

Write a JSON file at `/home/user/edp/data/persona_drafts/bargain_hunter_returning.json` with this EXACT schema:

```json
{
  "name": "bargain_hunter_returning",
