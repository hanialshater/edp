...
We demonstrate the primitive on page composition: at each session, choose and order 6 widgets out of 22 candidates given 14 raw behavioural signals and 6 fashion categories, under the reward signal production actually has (page-level attribution, multi-day delay, observation noise). The empirical contribution is that the closure properties do not cost performance. Against eight baselines on two simulators (parametric 8-persona; LLM-driven 14-persona × 6-category) under production reward conditions, EDP variants reach 6.5 ± 0.2 % oracle-regret on parametric (EDP-agent) and 11.0 ± 0.2 % on LLM-persona (Bayesian-EDP), roughly 2× better than the strongest combinatorial bandit we measured. We are deliberate about *why*: a decomposition (§5.4c) shows the honest comparator is not per-slot LinTS — which is the wrong abstraction under page-level reward — but **GreedyLinTS**, the same GAM architecture with no LLM, which already closes most of the gap (12.8 % / 13.3 %). The LLM's measured contribution on top is a competent zero-data prior and, on the harder simulator, an SGD-fused refinement; on the primary (LLM) simulator the LLM *editing* loop does not beat the architecture alone. We therefore frame the result as an existence proof that a readable, LLM-authorable primitive can match the strongest baselines we tried — not as an LLM-beats-bandits headline. The thesis is what the primitive enables in deployment: cold-start with zero training data (§5.7), zero serving dependencies, inline audit, and well-typed agent edits — none of which any bandit in the comparison supports.
...
Against eight baselines (per-slot LinTS warm and cold context, Slate-LinTS, CombLinUCB, GreedyLinTS, static placement, one-shot LLM-as-policy, OPRO ablation) on two simulators, the EDP variants are competitive. Bayesian-EDP loses 11.0 ± 0.2 % of oracle reward on the LLM-persona simulator (the primary setting); EDP-agent loses 6.5 ± 0.2 % on the parametric simulator (the robustness check). Neither EDP variant is uniformly best — the winner is simulator-conditional (§5.4c) — and both are reported against their best simulator, which the reader should weigh accordingly. Both are roughly 2× better than the strongest combinatorial bandit (CombLinUCB at 17.3 % / 10.5 %). We frame this as an existence proof that the primitive can compete, not a horse race. Crucially, §5.4c shows the honest comparator is GreedyLinTS — the same GAM architecture with no LLM, at 12.8 % / 13.3 % — which already closes most of the gap; the LLM contributes a zero-data prior and an SGD-fused refinement, not a categorical win over the architecture. EDP's closure properties don't cost performance. That is the empirical contribution.
...
§2 specifies the simulators, §3 details the 2-layer GAM and the evolution loop, and §5 reports results. Appendix A walks one session through the full open-box trace.

**Contributions:**

1. **The GAM-as-Software-3.0-primitive claim (§3).** A 200–300-parameter named GAM is the smallest policy representation with closure under both human and LLM editing. We quantify the property: 245 learnable parameters, 9 KB policy JSON, ~900-token diagnostic report, 15 well-typed edits per checkpoint round.
2. **GAM-as-prior Bayesian fusion (§5.4b).** Regularised SGD anchored at the LLM-authored shape functions. Best method on the LLM-persona simulator at 11.0 ± 0.2 %. The fusion preserves the readable representation while consuming online reward signal between checkpoints.
3. **Existence proof that the primitive can compete (§5).** Against eight baselines on two simulators under production reward conditions, EDP variants are within 2× of optimal and 2× better than the strongest combinatorial bandit. The closure properties do not cost performance; that is the empirical contribution.
4. **Direct measurement of the academic-vs-production gap (§5.1) and OPRO-style ablation (§5.4).** Per-slot LinTS degrades by 14 pp under page-level attribution. The gap is structural, not method-specific. The OPRO ablation isolates structured context, not the LLM's general intelligence, as the carrier of the LLM-side gain.

### 1.1 Related work and positioning

EDP sits at the intersection of four lines of work; none of its ingredients are new, and we position against each explicitly.
...
We treat **LLM-persona as the primary simulator** and parametric as a robustness check; results tables put LLM-persona first throughout. Bayesian-EDP loses 11.0 ± 0.2 % of oracle reward on LLM-persona; EDP-agent loses 6.5 ± 0.2 % on parametric; both are roughly 2× better than the strongest combinatorial bandit. The remainder of the section unpacks where that gap comes from: §5.1 measures the lab-to-production gap that motivates the primitive, and §5.4c decomposes the EDP gain into architecture, LLM-anchored prior, and continuous SGD. Where a subsection's claim is supported by a single-trial result rather than a multi-seed mean, we say so in line.
...
All bandits run at a single exploration setting (`α = 0.3`); we did not sweep `α` per condition. A condition-specific sweep could shift the absolute numbers and is the right way to make the bandit baselines maximally competitive — we flag it as a limitation (§7). The qualitative conclusion (every per-arm variant loses 8–15 pp moving from lab to production) is robust to `α` because the cause is the credit-assignment structure, not the exploration rate; but the precise CombLinUCB-vs-Slate-LinTS ordering below should be read with the no-sweep caveat in mind.
...
We present this as a **conjecture, not a result**. Two caveats weaken it. First, the inversion is a 0.8 pp gap (17.3 vs 16.5) at a single, un-swept `α`; it could be tuning noise rather than a structural effect, and only a per-condition `α` sweep would settle that. Second, the UCB-vs-TS asymmetry is not derived: Thompson sampling's posterior is *also* miscalibrated under correlated per-slot targets (indeed Bayesian-EDP needs heavy regularisation for exactly this reason, §5.4b), so it is not obvious a priori that TS's sampling is more robust than UCB's bonus to the residual-variance floor. A formal version would derive the floor and the resulting regret scaling for both rules. We have only the empirical inversion and offer the variance-floor story as a hypothesis to motivate that analysis, not as an established mechanism. What *is* robust across §5.2–5.3 is the direction: every per-arm method loses heavily moving to page-level reward.
